			...
			<table id='blog'>
				<caption>
					...
					<div class='insert' style='display:block;float:right;position:relative;text-transform:lowercase;width:210px'>
						<form action="<?php echo base_url() ?>admin/rental/search" method="post" accept-charset="utf-8" style='float:right;margin:-8px 0'>
...
button : \"cx_tanggal1\" // ID of the button
				...
				<tbody>
					<?php $num = 1; ?>
					<?php foreach($persewaan as $row): ?>
					<tr>
						<td><?php echo $num ?></td>
						<td><?php echo $row->id_persewaan ?></td>
						...
						<td><?php echo number_format($row->tarif,0,",",".") ?></td>
							<td>
							<a href="<?php echo base_url() ?>admin/rental/printr/<?php echo $row->id_persewaan ?>" target='_blank'>
								<img src="<?php echo base_url() ?>images/check.gif" alt="View Data" title="View Data">
							</a>
							
							<a href="<?php echo base_url() ?>admin/rental/delete_persewaan/<?php echo $row->id_persewaan ?>" onclick="return confirm('Apakah kamu yakin untuk menghapus invoice ini?')">
